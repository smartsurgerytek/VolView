<script lang="ts">
import vtkDentalWidget, {
  InteractionState,
  vtkDentalViewWidget,
  vtkDentalWidgetState,
} from '@/src/vtk/DentalWidget';
import {
  reactive,
  computed,
  defineComponent,
...
import DentalSVG2D from '@/src/components/tools/dental/DentalSVG2D.vue';
import {
...
export default defineComponent({
  ...
  props: {
    ...
    lineType: {
      type: String as PropType<'TRL' | 'CAL'>,
      default: 'TRL',
    },
  },
  components: {
  ...
  setup(props, { emit }) {
    ...
    const dentalLine = computed(() => dentalStore.toolByID[toolId.value]);
    const { metadata: imageMetadata } = useImage(imageId);

    const widgetFactory = vtkDentalWidget.newInstance({
      id: toolId.value,
    ...
    watchEffect(() => {
      ...
      updatePlaneManipulatorFor2DView(
        manipulator,
        viewDirection.value,
        dentalLine.value?.slice ?? slice.value,
        imageMetadata.value
      );
    });
    ...
    const isVisible = computed(() => dentalLine.value?.slice === slice.value);
    ...
    const updateVisibleState = (widgetState: vtkDentalWidgetState) => {
    ...
    return {
      ...
      slice,
      firstPoint: computed(() => {
        return visibleStates.firstPoint ? dentalLine.value?.firstPoint : undefined;
      }),
      secondPoint: computed(() => {
        return visibleStates.secondPoint ? dentalLine.value?.secondPoint : undefined;
      }),
      length: computed(() => dentalStore.lengthByID[dentalLine.value.id]),
    };
  },
});  
</script>
...
<template>
  <DentalSVG2D v-show="slice === dentalLine.slice" :point1="firstPoint" :point2="secondPoint" :color="dentalLine.color"
    :strokeWidth="dentalLine.strokeWidth" :length="length" :line-type="lineType" />
</template>